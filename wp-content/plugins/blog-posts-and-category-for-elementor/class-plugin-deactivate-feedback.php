...
if( ! class_exists( 'PD_PCF_Usage_Feedback') ) {
	
	class PD_PCF_Usage_Feedback {
		
		private $pd_pcf_version = '0.9.0';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function init() {
			global $pagenow;
				...
				add_action( 'wp_ajax_pd_pcf_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function admin_enqueue_scripts(){
			wp_enqueue_style('pdpcf-admin-style', PD_PCF_URL . 'admin/assets/css/admin.css', array(), PD_PCF_VERSION, 'all');

			$css='';
			$css .= '.button.pd_pcf-promo-link {
		      color: #ff0000;
		      font-weight: normal;
		      margin-left: 0;
			...
			wp_add_inline_style( 'pdpcf-admin-style', $css );
		}
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('Elementor Blog Post Filter Plugin Deactivation Notice');
				$headers = array('From: Elementor Blog Post Filter <mailer@just-a-fake-from-address.com>');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'pd_pcf_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'pd_pcf_deactivation_reason_' . $this->plugin_name );
				delete_option('pd_pcf_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'pd_pcf_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'pd_pcf_deactivation_details_' . $this->plugin_name );
				delete_option('pd_pcf_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'pd_pcf_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'pd_pcf_deactivation_email_' . $this->plugin_name );
				delete_option('pd_pcf_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'pd_pcf_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'pd_pcf_deactivation_main_reason_' . $this->plugin_name );
				delete_option('pd_pcf_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-pd_pcf-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'pd_pcf_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				$html .= '<div class="pd_pcf-goodbye-options"><p>';
				foreach( $form['options'] as $option ) {
					$html .= '<input type="radio" name="pd_pcf-goodbye-options" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><div id="pd_pcf_additional_content" style="display:none;"><label for="pd_pcf-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="pd_pcf-goodbye-email" id="pd_pcf-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				
				$html .= '<br><label for="pd_pcf-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="pd_pcf-goodbye-reasons" id="pd_pcf-goodbye-reasons" rows="2" style="width:100%"></textarea></div>';
				$html .= '</div><!-- .pd_pcf-goodbye-options -->';
			}
			...
			?>
			<div class="pd_pcf-goodbye-form-bg"></div>


			<script>
				jQuery(document).ready(function($){
					 $('.pd_pcf-goodbye-options input[type=radio]').on('change', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#pd_pcf_additional_content').hide();
						}else{
							$('#pd_pcf_additional_content').show();
						}
						
					 });

					$('#wpb-goodbye-link-pd_pcf-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-pd_pcf-<?php echo esc_attr( $this->plugin_name ); ?>');
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							$.each($('input[name=\'pd_pcf-goodbye-options[]\']:checked'), function(){
								values.push($(this).val());
							});
							var email = $('#pd_pcf-goodbye-email').val();
							var details = $('#pd_pcf-goodbye-reasons').val();
							var deactivate_main_reason = $('input[name="pd_pcf-goodbye-options"]:checked').val();
							var data = {
								'action': 'pd_pcf_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'pd_pcf_goodbye_form' ); ?>',
								'dataType': 'json'
							}
						...
						$('.pd_pcf-goodbye-form-bg').on('click',function(){
		...
		public function goodbye_form_callback() {
			//check_ajax_referer( 'pd_pcf_goodbye_form', 'security' );
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'pd_pcf_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'pd_pcf_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'pd_pcf_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'pd_pcf_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
